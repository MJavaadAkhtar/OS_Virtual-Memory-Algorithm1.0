...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		//Firs tfind the dirtty copy inorder to see whether swapping is
		// necessary or not
	    if (PG_DIRTY & coremap[frame].pte->frame){
			if ((PG_ONSWAP & coremap[frame].pte->frame)==0)
				coremap[frame].pte->frame =  coremap[frame].pte->frame | PG_ONSWAP;
			// swapping out now
			coremap[frame].pte->frame = (coremap[frame].pte->frame & ~PG_VALID);
            int swap=swap_pageout(frame, coremap[frame].pte->swap_off);
            if(swap==INVALID_SWAP){
                perror("Swapping Error, the swap is Invalid");
                exit(EXIT_FAILURE);
            }
            if(coremap[frame].pte->swap_off == INVALID_SWAP)
            	coremap[frame].pte->swap_off = swap;

			evict_dirty_count++;

		}else{
			// if onswap is not set, then set it
			if((coremap[frame].pte->frame & PG_ONSWAP)==0)
            	coremap[frame].pte->frame =  coremap[frame].pte->frame | PG_ONSWAP;
			// If the swap is not invalid
			if((coremap[frame].pte->swap_off)!=INVALID_SWAP){
				coremap[frame].pte->frame = coremap[frame].pte->frame | PG_ONSWAP;
				coremap[frame].pte->frame = (coremap[frame].pte->frame & ~PG_VALID);
				evict_clean_count++;
			}else{
				//if the swap is invalid
				// now we set the PG_ONSWAP
				coremap[frame].pte->frame=coremap[frame].pte->frame | PG_ONSWAP;
                coremap[frame].pte->frame = (coremap[frame].pte->frame & ~PG_VALID);
				int swap=swap_pageout(frame, coremap[frame].pte->swap_off);
                if(swap==INVALID_SWAP){
		...
		// This is checking if it's onswap
		if (p->frame & PG_ONSWAP){
			int swap = swap_pagein(frame, p->swap_off);
			if(swap!=0){
				perror("Swapping Error, the swap is Invalid");
				exit(EXIT_FAILURE);
			}
	...
	// since the code is reached here, we have to set the p frame to a valid one
...
//	(void)idx; // To keep compiler happy - remove when you have a real use.
	// initialize pde if it is invalid, doing the check in the if condition
	if (((pgdir[idx].pde)&~PG_VALID)==0)
		pgdir[idx] = init_second_level();


	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	pgtbl_entry_t *new= (pgtbl_entry_t *)((pgdir[idx].pde)&~PG_VALID);
    p = &(new[PGTBL_INDEX(vaddr)]);
